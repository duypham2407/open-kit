...
| `openkit configure mcp ...` | available | `global_cli` | operators | guided interactive MCP setup, inspecting bundled MCPs/skills, enabling/disabling MCPs, storing local MCP keys, repairing secret-store permissions, and testing MCP readiness | `openkit configure mcp --interactive` is TTY-only and wraps the existing control plane; see `docs/operator/mcp-configuration.md`; outputs and profiles must use placeholders/redaction only |
