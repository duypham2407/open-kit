...
| `tool.skill-index` / `tool.skill-mcp-bindings` / skill routing | available | `runtime_tooling` | operators/agents/maintainers | inspecting bundled skill metadata, maturity status, support level, provenance, roles/stages/triggers, and advisory MCP linkage | skill `status` is `stable`/`preview`/`experimental`; runtime `capabilityState` is separate; routing recommends and explains, it does not silently activate skills |
