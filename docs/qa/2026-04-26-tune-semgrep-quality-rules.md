...
- Semgrep regression harness and fixtures under `src/tests/semgrep/` and `src/tests/fixtures/semgrep/`
...
| Regression suite asserts positive and negative fixture behavior by rule id | PASS | `src/tests/semgrep/quality-rules.test.js` normalizes path-prefixed rule ids via suffix matching and asserts positive, negative-modern, negative-text/metadata, mixed, and security-sanity behavior. |
...
| No target-project app validation claim | PASS | `docs/operator/semgrep.md`, `src/context/core/project-config.md`, `AGENTS.md`, and this QA report state OpenKit Semgrep evidence validates `runtime_tooling`/governance only, not target app build/lint/test behavior. |
...
- security-scan: direct runtime security-scan probe succeeded on the same 9 surfaces; 1 total finding on the intentional `src/tests/fixtures/semgrep/security/security-sanity.js` fixture (`openkit.security.no-new-function`), 0 findings on implementation/doc/package/quality fixture surfaces.
...
- `docs/operator/semgrep.md`, `docs/maintainer/test-matrix.md`, `src/context/core/project-config.md`, and `AGENTS.md` document the Semgrep regression gate and validation-surface boundary.
