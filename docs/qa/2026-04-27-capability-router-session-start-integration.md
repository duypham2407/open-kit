...
| Role/stage guardrails | PASS | `src/tests/runtime/capability-tools.test.js` covers MasterOrchestrator, ProductLead, SolutionLead, FullstackAgent, CodeReviewer, QAAgent, QuickAgent, and migration guardrails. Active-state smoke showed `full / full_qa / QAAgent` guidance with QA-only verification/evidence wording. |
| Session-start integration | PASS | `src/openkit-runtime/tests/session-start-hook.test.js` passed 8/8. Manual smoke with active managed FEATURE-949 state printed `<openkit_runtime_status>`, then `<openkit_capability_guidance>`, then resume hints; the guidance block included startup snapshot and refresh routes. |
| Runtime summary exposure | PASS | `src/openkit-runtime/tests/workflow-state-cli.test.js` and `src/tests/runtime/runtime-platform.test.js` passed. `resume-summary --json` exposed `capability_guidance` / `capability_guidance_lines`; source runtime-summary/controller spot checks exposed `capabilityGuidance` / `capabilityGuidanceLines` with `runtime_tooling` labels. |
...
| Direct runtime `tool.rule-scan` over FEATURE-949 changed files | `runtime_tooling` | 0 | 24 files scanned; 336 warnings, all from `src/openkit-runtime/workflow-state.js` CLI terminal output and triaged non-blocking. |
| Direct runtime `tool.security-scan` over FEATURE-949 changed files | `runtime_tooling` | 0 | 24 files scanned; 1 test-fixture secret warning in `src/tests/runtime/runtime-platform.test.js`, triaged false positive. |
...
- syntax-outline: attempted for changed source files and project-wide structural expectations, but unavailable/degraded in this QA namespace due path resolver returning `missing-file`/`invalid-path` for the active root and `fileCount: 0` project-wide. Substituted with targeted source reads plus structural assertions in `src/tests/runtime/capability-tools.test.js`, `src/openkit-runtime/tests/session-start-hook.test.js`, `src/openkit-runtime/tests/workflow-state-cli.test.js`, and `src/tests/runtime/runtime-platform.test.js`.
...
| `tool.security-scan` | available/succeeded; 24 changed implementation/doc files scanned through OpenKit runtime tool harness | none for scan execution; QA triage applied to fresh finding | 1 total error-severity finding | blocking 0, true_positive 0, non_blocking_noise 0, false_positive 1 group, follow_up 0, unclassified 0 | Test fixture placeholder in `src/tests/runtime/runtime-platform.test.js` verifies env substitution; it is not a real credential, is not production config, and has no runtime security impact. | none | `runtime_tooling`; stored evidence is `compatibility_runtime`; not `target_project_app` | `.openkit/artifacts/feature-949-direct-scan-details.json`; evidence `feature-949-qa-direct-scans-2026-04-27` |
...
- Direct scan noise: `src/openkit-runtime/workflow-state.js` intentionally uses `console.log` for CLI output; this is covered by workflow-state CLI regression tests and is non-blocking.
