...
- `compatibility_runtime`: `src/openkit-runtime/workflow-state.js` validation, doctor output, active work item state, task-board validation, issue/evidence state, and runtime summary tooling.
...
- `security-scan`: direct `tool.security-scan` invocation was unavailable in this QA tool namespace; substitute Semgrep security scan ran against the same 39 files using `assets/semgrep/packs/security-audit.yml` and returned 1 finding. QA triaged it as a non-blocking test-fixture false positive: `src/tests/runtime/runtime-platform.test.js:162` contains the placeholder string `${TEST_TOKEN}` in an MCP config fixture, not a real secret, and the FEATURE-938 diff in that file only adds `OPENKIT_WORKFLOW_STATE` around line 327.
...
| AC2.2 Tool Availability States Are Explicit | PASS | `src/runtime/capability-registry.js`, `src/context/core/project-config.md`, `src/context/core/runtime-surfaces.md`, and operator/maintainer docs use the standardized vocabulary (`available`, `unavailable`, `degraded`, `preview`, `compatibility_only`, `not_configured`). Runtime foundation and capability-registry tests passed. |
| AC2.3 Command Reality Stays Aligned With Implemented Behavior | PASS | `package.json` scripts, `src/bin/openkit.js`, `src/openkit-runtime/workflow-state.js`, command matrix docs, operator docs, and governance tests align current commands with implemented behavior. `npm run verify:all` and `npm run verify:governance` passed. |
...
| AC3.2 Role Boundaries Remain Visible | PASS | `src/context/core/workflow.md`, `docs/maintainer/README.md`, `docs/maintainer/2026-03-26-role-operating-policy.md`, and `docs/maintainer/parallel-execution-matrix.md` preserve Product Lead, Solution Lead, Fullstack, Code Reviewer, QA Agent, and Master Orchestrator boundaries. Master Orchestrator remains route/state/gate control only. |
...
2. **Semgrep security false positive in test fixture**
   - Evidence: substitute security scan returned one `openkit.security.no-hardcoded-secret` finding at `src/tests/runtime/runtime-platform.test.js:162`, where the fixture writes JSON containing placeholder `${TEST_TOKEN}`. The FEATURE-938 diff in that file only adds `OPENKIT_WORKFLOW_STATE: statePath` around line 327.
