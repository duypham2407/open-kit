...
> **For agentic workers:** REQUIRED: Use `src/skills/subagent-driven-development/SKILL.md` when subagents are available. Steps use checkbox (`- [ ]`) syntax for tracking.
...
- Canonical workflow docs: `AGENTS.md`, `src/context/core/workflow.md`, `src/context/core/session-resume.md`, `src/context/core/workflow-state-schema.md`, `src/context/core/project-config.md`
- Runtime utilities: `src/openkit-runtime/workflow-state.js`, `src/openkit-runtime/lib/workflow-state-controller.js`, `src/openkit-runtime/lib/workflow-state-rules.js`, `src/openkit-runtime/lib/contract-consistency.js`
- Existing runtime tests: `src/openkit-runtime/tests/workflow-state-controller.test.js`, `src/openkit-runtime/tests/workflow-state-cli.test.js`, `src/openkit-runtime/tests/workflow-behavior.test.js`, `src/openkit-runtime/tests/workflow-contract-consistency.test.js`, `src/openkit-runtime/tests/session-start-hook.test.js`
- Existing validation command: `node --test ".opencode/tests/*.test.js"`

## Planned File Map

- **Work-item storage layer**
  - Create: `src/openkit-runtime/lib/work-item-store.js`
...
- **Task-board rules layer**
  - Create: `src/openkit-runtime/lib/task-board-rules.js`
  - Modify: `src/openkit-runtime/lib/workflow-state-rules.js`
  - Modify: `src/openkit-runtime/lib/workflow-state-controller.js`
- **Schema and workflow docs layer**
  - Modify: `src/context/core/workflow.md`
  - Modify: `src/context/core/workflow-state-schema.md`
  - Modify: `src/context/core/session-resume.md`
  - Modify: `src/context/core/project-config.md`
  - Modify: `AGENTS.md`
  - Modify: `README.md`
- **Diagnostics and operator layer**
  - Modify: `src/openkit-runtime/lib/contract-consistency.js`
  - Modify: `src/hooks/session-start`
  - Modify: `docs/operations/README.md`
  - Modify: `docs/operations/runbooks/workflow-state-smoke-tests.md`
  - Modify: `README.md`
- **Test layer**
  - Create: `src/openkit-runtime/tests/work-item-store.test.js`
  - Create: `src/openkit-runtime/tests/task-board-rules.test.js`
  - Create: `src/openkit-runtime/tests/multi-work-item-runtime.test.js`
  - Modify: `src/openkit-runtime/tests/workflow-state-controller.test.js`
  - Modify: `src/openkit-runtime/tests/workflow-state-cli.test.js`
  - Modify: `src/openkit-runtime/tests/workflow-contract-consistency.test.js`
  - Modify: `src/openkit-runtime/tests/session-start-hook.test.js`

## Execution Rules

- Do not change the quick-lane contract or add task-board runtime to quick mode.
- Keep `src/openkit-runtime/workflow-state.json` as the external canonical interface for phase 1 compatibility.
...
- The repository already contains `src/openkit-runtime/workflow-state.json` as the checked-in active work-item example.
- Phase 1 must bootstrap from that existing file into `src/openkit-runtime/work-items/index.json` plus `src/openkit-runtime/work-items/<work-item-id>/state.json` without breaking existing entry commands.
- `start-feature` and `start-task` must remain backward-compatible entrypoints: they should create or select a work item, write per-item state first, then refresh `src/openkit-runtime/workflow-state.json` as the active compatibility mirror.
...
- bootstrap from the checked-in `src/openkit-runtime/workflow-state.json` should derive `work_item_id` from `feature_id` unless a stored work-item id already exists
- `src/openkit-runtime/work-items/index.json` must contain at least:
  - `active_work_item_id`
...
Create `src/openkit-runtime/tests/work-item-store.test.js` with cases for:
- bootstrapping an existing `src/openkit-runtime/workflow-state.json` into `src/openkit-runtime/work-items/index.json` and `src/openkit-runtime/work-items/<id>/state.json`
- deriving a stable `work_item_id` and avoiding duplicate bootstrap entries across repeated runs
- creating `src/openkit-runtime/work-items/index.json`
- creating `src/openkit-runtime/work-items/<id>/state.json`
- activating one work item and mirroring it into `src/openkit-runtime/workflow-state.json`
- reporting divergence when the active-item mirror is stale or missing

Add focused failing coverage in `src/openkit-runtime/tests/workflow-state-controller.test.js` for reading and writing the active work item through the compatibility layer.
...
Create `src/openkit-runtime/lib/work-item-store.js` with narrow responsibilities:
- bootstrap a legacy `src/openkit-runtime/workflow-state.json` into the phase-1 work-item layout
- derive and persist stable `work_item_id` values plus the minimal `index.json` shape
- resolve `src/openkit-runtime/work-items/index.json`
- read and write the work-item index
- read and write one work item state file
- update the active-item pointer in the index
- refresh `src/openkit-runtime/workflow-state.json` as the active compatibility mirror
...
Update `src/openkit-runtime/lib/workflow-state-controller.js` so feature-level operations use the active work-item backing store while preserving current command behavior for callers that still target `src/openkit-runtime/workflow-state.json`.
...
Extend `src/openkit-runtime/workflow-state.js` with concrete commands:
...
Create `src/openkit-runtime/tests/task-board-rules.test.js` and `src/openkit-runtime/tests/multi-work-item-runtime.test.js` with cases for:
...
Create `src/openkit-runtime/lib/task-board-rules.js` to own:
...
Update `src/openkit-runtime/lib/workflow-state-controller.js` so `full` work items can carry `tasks.json` under `src/openkit-runtime/work-items/<id>/tasks.json`, while `quick` work items reject task-board operations.
...
- Modify: `src/openkit-runtime/lib/workflow-state-controller.js`
- Modify: `src/openkit-runtime/tests/workflow-state-cli.test.js`
- Modify: `src/openkit-runtime/tests/multi-work-item-runtime.test.js`
...
- `src/openkit-runtime/workflow-state.json` still canonical externally in phase 1
...
Make `src/context/core/workflow.md` and `src/context/core/workflow-state-schema.md` agree on:
- the new multi-work-item runtime model
- the compatibility role of `src/openkit-runtime/workflow-state.json`
...
Make `src/context/core/session-resume.md` describe how full-delivery resume should surface feature context plus assigned task context.

Make `src/context/core/project-config.md` list the new runtime commands and limits honestly.
...
- confirm no doc says `src/openkit-runtime/workflow-state.json` disappeared in phase 1
...
Update `src/openkit-runtime/lib/contract-consistency.js` to validate only stable multi-item invariants:
...
- `README.md`
- `src/openkit-runtime/workflow-state.js --help` output

Confirm they tell the same story about:
- quick mode staying simple
- full-delivery-only task boards
- compatibility mirroring through `src/openkit-runtime/workflow-state.json`
