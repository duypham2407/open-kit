...
- `src/skills/writing-solution/SKILL.md`
...
- `src/skills/writing-scope/SKILL.md`
...
- `src/openkit-runtime/lib/parallel-execution-rules.js`
...
- `src/openkit-runtime/lib/contract-consistency.js`
...
- Runtime hardcoding drift
  - Grep and replace all old full-stage and gate names in `src/openkit-runtime/lib/` before running tests.

- Registry and bundle drift
  - Update `registry.json`, `src/install/asset-manifest.js`, `src/tests/install/install-state.test.js`, and bundled mirrors before closing the refactor.
...
- Planning skill drift
  - Rewrite `src/skills/writing-solution/SKILL.md` so `Solution Lead` outputs feature-level slices, dependencies, and validation strategy instead of defaulting to micro-tasks.

- Review routing drift
  - Update `src/agents/code-reviewer.md`, `src/context/core/issue-routing.md`, and `workflow-state-rules.js` together so every reviewer finding maps to a concrete workflow route.
