...
- **Acceptance criteria:**
  ...
  - [ ] Test added/extended: a regression test under `tests/release/` (or `tests/runtime/registry-metadata.test.js`) that asserts `registry.kit.version === pkg.version` and `install-manifest.kit.version === pkg.version`
...
- **Acceptance criteria:**
  ...
  - [ ] New test `tests/runtime/fsm-table-consistency.test.js` asserts deep-equal of per-mode transition maps (covers [X-2])
  - [ ] Existing `tests/runtime/advance-stage.test.js` and `.opencode/tests/workflow-state-controller.test.js` continue to pass
...
- **Acceptance criteria:**
  ...
  - [ ] New test `tests/runtime/ast-grep-search-injection.test.js` asserts a pattern containing shell metacharacters returns no matches and does not execute the metacharacters (e.g., does not create a side-effect file)
...
- **Priority:** High
- **Location:** `registry.json` (no entry); `commands/configure-embedding.md:1`
- **Root cause:** Command file added without a corresponding registry entry.
- **Fix approach:** Insert an entry under `components.commands` matching the schema used by other commands: `{ "id": "command.configure-embedding", "path": "commands/configure-embedding.md", "audience": "operator", … }`. Verify `tests/runtime/registry-metadata.test.js` enumerates from `registry.json` and that the new entry passes whatever schema check the test runs.
- **Acceptance criteria:**
  ...
  - [ ] `tests/runtime/registry-metadata.test.js` passes
...
- **Acceptance criteria:**
  ...
  - [ ] New test `tests/contract/agent-tool-references.test.js` parses each agent markdown and asserts every `tool.<id>` reference exists in the registry
...
- **Acceptance criteria:**
  ...
  - [ ] New test `tests/mcp-server/tool-schemas.test.js` (or extension of existing) asserts every runtime-registered tool has a matching MCP schema entry
...
- **Acceptance criteria:**
  ...
  - [ ] New test `tests/runtime/workflow-kernel-error-propagation.test.js` injects a throwing controller and asserts the error is observable to the caller
  - [ ] Existing `tests/runtime/workflow-kernel-integration.test.js` passes
...
- **Acceptance criteria:**
  ...
  - [ ] New test `tests/runtime/gate-system-coverage.test.js` asserts every transition in TRANSITIONS that requires a gate has a matching `GateRegistry` entry
  - [ ] `tests/runtime/advance-stage.test.js` and `.opencode/tests/workflow-state-controller.test.js` pass
...
- **Acceptance criteria:**
  ...
  - [ ] New test `tests/global/doctor-sub-check-propagation.test.js` injects an unhealthy sub-check and asserts exit code 1
  - [ ] Existing `tests/global/doctor.test.js` passes
...
- **Acceptance criteria:**
  - [ ] `tests/install/merge-policy.test.js` adds a case where two structurally-identical objects are merged into a single entry
...
- **Acceptance criteria:**
  - [ ] New test `tests/global/upgrade-atomic.test.js` simulates a copy failure mid-way and asserts `kitRoot` still has the original content
  - [ ] Successful upgrade leaves `kitRoot` with new content and no leftover temp/`.prev` dirs
  - [ ] `tests/global/doctor.test.js` continues to pass
...
- **Acceptance criteria:**
  - [ ] New test `tests/cli/upgrade-error-handling.test.js` injects a throwing materialize and asserts exit code 1 + a non-empty stderr message
...
- **Acceptance criteria:**
  - [ ] New test `tests/runtime/openclaw-command-validation.test.js` asserts that crafted commands (relative path, with shell metas) are rejected at config load
...
- **Acceptance criteria:**
  - [ ] New test `tests/runtime/external-mcp-command-validation.test.js` asserts crafted commands are rejected with a clear error
...
- **Acceptance criteria:**
  - [ ] New test `tests/global/keychain-cli-validation.test.js` sets `OPENKIT_SECURITY_CLI=/tmp/evil` and asserts the adapter falls back to `/usr/bin/security`
...
- **Acceptance criteria:**
  - [ ] New test `tests/runtime/prompt-file-loader-boundary.test.js` asserts `file:///etc/passwd` and `file://~/secret` are rejected
...
- **Fix approach:** Add `tests/runtime/graph-indexer-e2e.test.js` that creates a fixture project (a small git tree with a few JS/TS files), spawns `node hooks/graph-indexer.js` against it, and asserts the resulting DB contains expected nodes/edges. Use existing fixture utilities under `tests/runtime/`.
- **Acceptance criteria:**
...
- **[4-M-3]** Add redaction pass to `invocationLogger.record` for the `result` field — `.opencode/lib/invocation-log.js:156` — effort: M
...
- **[4-M-5]** Expand `tests/semgrep/quality-rules.test.js` with positive/negative fixtures for each existing security rule — effort: M
...
- **[1-L-3]** Document `JSON.stringify` non-canonicalization caveats in `state-guard.js` (or use a canonical-json lib) — `.opencode/lib/state-guard.js:10-28` — effort: S (doc only)
...
- **[2-L-3]** Wrap `bin/openkit-mcp.js` import in a startup-error handler that emits stderr — effort: S
- **[3-L-1]** Add `(Solution Lead)` annotation to `migration_baseline` row in `commands/migrate.md:46` — effort: S
...
- **Refactor of files > 500 lines** — `.opencode/lib/workflow-state-controller.js` (≈ 69K tokens) is large and was only partially read by Subagent 1. Splitting it into focused modules is a separate workstream.
...
|----------|------------------------------|
| [D-1]    | `npm run verify:all` + new `tests/release/version-metadata-consistency.test.js` |
| [1-C-1]  | `tests/runtime/fsm-table-consistency.test.js` (new) + existing `advance-stage.test.js` |
| [1-C-2]  | `tests/mcp-server/workflow-state-contract.test.js` (new) |
| [4-C-1]  | `tests/runtime/ast-grep-search-injection.test.js` (new) + semgrep rule |
| [1-H-1]  | `tests/runtime/workflow-kernel-error-propagation.test.js` (new) |
| [1-H-2]  | `tests/runtime/gate-system-coverage.test.js` (new) + advance-stage / controller tests |
| [1-H-3]  | extension of `tests/mcp-server/mcp-server.test.js`; manual MCP smoke |
| [2-H-1]  | `tests/global/upgrade-atomic.test.js` (new) |
| [2-H-2]  | `tests/global/doctor-sub-check-propagation.test.js` (new) |
| [2-H-3]  | extension of `tests/install/merge-policy.test.js` |
| [2-H-4]  | `tests/cli/upgrade-error-handling.test.js` (new) |
| [3-H-2]  | `tests/runtime/registry-metadata.test.js` (existing) |
| [3-H-3]  | `tests/contract/agent-tool-references.test.js` (new) |
| [4-H-1]  | `tests/runtime/openclaw-command-validation.test.js` (new) |
| [4-H-2]  | `tests/runtime/external-mcp-command-validation.test.js` (new) |
