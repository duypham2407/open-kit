...
- **[D-1]** Version drift: `package.json` is `0.5.1` but `registry.json` and `src/openkit-runtime/install-manifest.json` still show `0.3.36`
  - Source IDs: `[2-C-1] + [3-H-1] + [2-L-1]`
  - Files: `package.json:3`, `registry.json:6`, `src/openkit-runtime/install-manifest.json:6`
...
- [3-H-2] `src/commands/configure-embedding.md` exists on disk but absent from `registry.json` — `src/commands/configure-embedding.md:1`
- [3-H-3] Agents reference `tool.heuristic-lsp` which is not registered — `src/agents/solution-lead-agent.md:51`, `src/agents/code-reviewer.md:76`
...
- [1-L-3] `captureRevision` uses `JSON.stringify` (drops `undefined`) — `src/openkit-runtime/lib/state-guard.js:10-28`
...
- [4-L-2] `session-start.js` prints absolute paths on every session start — `src/hooks/session-start.js:271-283`
...
- **Result: clean.** Stages enumerated in `registry.json` match `STAGE_ORDER` in `src/runtime/state/transition-engine.js`. The `migration_report` token in `src/commands/write-solution.md:44,58` is an artefact name, not a stage. No mismatched stage names.
...
- The 3 wrapper-facing files in `src/openkit-runtime/install-manifest.json` (`registry.json`, `src/openkit-runtime/install-manifest.json`, `assets/install-bundle/opencode/skill-catalog.json`) are all in `pkg.files`.
...
- **Drift confirmed for `registry.json` (0.3.36) and `src/openkit-runtime/install-manifest.json` (0.3.36)** — already captured as `[D-1]`. No new finding here.

### Cross-layer check 5 — Test coverage of critical paths
- bootstrap: covered by `src/tests/runtime/lane-bootstrap-e2e.test.js`, `src/tests/runtime/bootstrap-workflow.test.js`, `src/tests/runtime/workflow-kernel-fresh.test.js` ✓
- merge-policy: covered by `src/tests/install/merge-policy.test.js` ✓
- session-start hook: covered by `src/openkit-runtime/tests/session-start-hook.test.js` ✓
- graph-indexer hook: **no test file** — already captured as `[4-H-5]`.
- doctor: covered by `src/tests/runtime/doctor.test.js` and `src/tests/global/doctor.test.js` ✓
- upgrade: only CLI-level mocked (`src/tests/cli/openkit-cli.test.js`) — already captured as `[4-L-3]`.
- **[X-2] Low** — FSM transition table itself has unit tests at `src/tests/runtime/advance-stage.test.js` but no test asserts the **two FSM tables** (`state-machine.js` vs `transition-engine.js`) **agree**, which is the root cause of `[1-C-1]`. A consistency test would have caught it.
  - Suggested fix: add `src/tests/runtime/fsm-table-consistency.test.js` that imports both tables and asserts deep-equal of their per-mode transition maps.
...
- [D-1] ✓ verified — `package.json:3` = "0.5.1", `registry.json` (kit.version field) = "0.3.36", `src/openkit-runtime/install-manifest.json:6` = "0.3.36".
