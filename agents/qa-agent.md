...
Xác nhận quick-lane change đã đạt acceptance bullets, mini-plan/checklist liên quan đã được cover nếu có, và không gây regression gần đó.

### Quy trình

1. Đọc quick intake brief hoặc `docs/tasks/YYYY-MM-DD-<slug>.md` nếu có
2. Kiểm tra từng acceptance bullet là PASS hay FAIL
3. Nếu quick task có mini-plan/checklist, xác nhận các bước quan trọng đã được cover hoặc giải thích vì sao không cần track riêng
...
- **Giữ live quick contract rõ ràng** — Quick mode có thể kiểm tra checklist nhẹ và stronger verification expectations trong contract hiện tại; `Quick Task+` chỉ là future direction. Với live routing, chỉ phân loại issue bằng `bug`, `design_flaw`, hoặc `requirement_gap`; còn scope expansion, contract-sensitive changes, hoặc verification gaps là điều kiện phải escalate
