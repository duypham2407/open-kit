...
test('materializeGlobalInstall configures managed MCP defaults', () => {
  ...
  const expectedOpenkitCommand = [
    process.execPath,
    path.join(process.cwd(), 'src', 'bin', 'openkit-mcp.js'),
  ];
...
test('package manifest publishes OpenCode plugin runtime surfaces', () => {
  ...
  // Plugins live under src/openkit-runtime/plugins/, covered by the top-level "src/" entry.
  assert.equal(packageManifest.files.includes('src/'), true);
});
