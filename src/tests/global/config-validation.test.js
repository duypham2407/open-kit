...
import { materializeMcpProfiles } from '../../global/mcp/profile-materializer.js';
import { materializeInstall } from '../../install/materialize.js';
import { applyOpenKitMergePolicy } from '../../install/merge-policy.js';
import { validateAgentModelSettings } from '../../runtime/config-validation.js';
import {
  validateOpenCodeConfigTopLevelKeys,
} from '../../opencode/config-schema.js';
...
test('checked-in OpenCode config projections are strict-schema-safe and preserve permissions', () => {
  const template = readJson(path.join(process.cwd(), 'src', 'assets', 'opencode.json.template'));
  const repoLocal = readJson(path.join(process.cwd(), 'src', 'openkit-runtime', 'opencode.json'));

  assertStrictOpenCodeConfig(template, 'src/assets/opencode.json.template');
  assertStrictOpenCodeConfig(repoLocal, 'src/openkit-runtime/opencode.json');
