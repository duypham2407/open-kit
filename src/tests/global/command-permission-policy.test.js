...
test('loadDefaultCommandPermissionPolicy can load a packaged policy path and reports malformed JSON', () => {
  ...
  const policyPath = path.join(tempRoot, 'src', 'assets', 'default-command-permission-policy.json');
  const policy = loadDefaultCommandPermissionPolicy();
...
test('repo-local compatibility configs mirror the policy projection', () => {
  const policy = loadDefaultCommandPermissionPolicy();
  const projection = buildOpenCodePermissionConfig(policy);
  const template = JSON.parse(fs.readFileSync(path.join(process.cwd(), 'src', 'assets', 'opencode.json.template'), 'utf8'));
  const repoLocal = JSON.parse(fs.readFileSync(path.join(process.cwd(), 'src', 'openkit-runtime', 'opencode.json'), 'utf8'));
