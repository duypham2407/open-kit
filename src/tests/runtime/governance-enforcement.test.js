...
const SOURCE_PATH_DIRECTORIES = new Set(['agents', 'commands', 'skills', 'context', 'assets', 'hooks']);

function resolveProjectFile(filePath) {
  const firstSegment = filePath.split('/')[0];
  if (SOURCE_PATH_DIRECTORIES.has(firstSegment)) {
    return path.join(sourceRoot, filePath);
  }
  return path.join(repoRoot, filePath);
}

function read(filePath) {
  return fs.readFileSync(resolveProjectFile(filePath), 'utf8');
}

function listMarkdownFiles(relativeDir) {
  return fs.readdirSync(resolveProjectFile(relativeDir))
...
test('semgrep quality regression docs keep unavailability fail-closed', () => {
  const semgrepTest = read('src/tests/semgrep/quality-rules.test.js');
...
test('canonical docs keep the managed path model and environment anchors explicit', () => {
  ...
  assert.match(surfaceContract, /keep the path model explicit: global kit root for managed kit\/config, workspace state root for active runtime state, project `src\/openkit-runtime\/` for compatibility shim behavior/);
});
