...
} from "../../install/asset-manifest.js"
import {
  INSTALL_STATE_SCHEMA,
  createInstallState,
  validateInstallState,
} from "../../install/install-state.js"
import { validateOpenCodeConfigTopLevelKeys } from "../../opencode/config-schema.js"
...
test("asset manifest defines the explicit OpenCode-native phase 1 bundle", () => {
  ...
  assert.deepEqual(listBundledAssetIds(), [
    ...
    "opencode.skill.writing-solution",
    "opencode.skill.writing-scope",
  ])
...
test("bundled asset validator reports drift between source and derived copy", () => {
  const testDir = path.dirname(fileURLToPath(import.meta.url))
  const projectRoot = path.resolve(testDir, "../../..")
  const bundledFilePath = path.join(projectRoot, "src/assets/install-bundle/opencode/commands/quick-task.md")
