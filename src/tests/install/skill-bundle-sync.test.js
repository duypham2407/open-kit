...
import { listInstallBundleSkillMetadata } from '../../capabilities/skill-catalog.js';
...
function makeValidationFixture() {
  ...
  copyDirectoryContents(path.join(projectRoot, 'src/skills'), path.join(fixtureRoot, 'src/skills'));
  copyDirectoryContents(path.join(projectRoot, 'src/assets/install-bundle'), path.join(fixtureRoot, 'src/assets/install-bundle'));
...
test('install bundle validation fails for an extra source skill without canonical metadata', () => {
  ...
  const extraSkillPath = path.join(fixtureRoot, 'src', 'skills', 'unmapped-source-skill', 'SKILL.md');
  ...
  assert.deepEqual(validation.missingSourceSkillCatalogEntries, [
    'src/skills/unmapped-source-skill/SKILL.md',
  ]);
  assert.deepEqual(validation.sourceSkillsMissingInstallBundleDecision, [
    'src/skills/unmapped-source-skill/SKILL.md',
  ]);
