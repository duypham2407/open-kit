...
- `npm run sync:version` syncs generated OpenKit package metadata mirrors from the canonical `package.json#version`. It updates package metadata surfaces only and is not target-project application validation.
...
- `openkit switch-profiles` and `openkit switch` are direct CLI pickers for switching an already-running session to an existing global profile. They require a valid `OPENKIT_RUNTIME_SESSION_ID`, fail closed outside an active OpenKit runtime session, write only session/workspace runtime state, and must not mutate the global profile store, global default, or other sessions.
- `/switch-profiles` is the in-session prompt-template path for the same interactive-only profile switch. OpenCode custom slash command files are prompt templates; true native executable slash command support is not currently documented, so this path may be agent-mediated rather than a guaranteed native command runner.

## Permission Policy

- The canonical machine-readable command permission policy is `assets/default-command-permission-policy.json` (`openkit/command-permission-policy@1`). Static OpenCode config files are strict-schema-safe `permission` projections of that policy, not separate policy sources.
- Global OpenKit-managed config is the primary product target. `openkit run`, first-time global install, and upgrade-style materialization write policy-derived permissions into `<OPENCODE_HOME>/kits/openkit/opencode.json` and `<OPENCODE_HOME>/profiles/openkit/opencode.json` without embedding OpenKit-only top-level metadata.
...
- OpenCode-validated `opencode.json` files must contain only OpenCode schema-valid top-level keys. OpenKit policy metadata such as `commandPermissionPolicy` remains in `assets/default-command-permission-policy.json` or OpenKit-owned sidecars, never inline in OpenCode-validated config.
...
- `openkit configure mcp custom add-remote <custom-id> --url <url> [--transport http|sse|streamable-http] [--header <Header=${ENV}> ...] [--env <ENV=${ENV}> ...] [--scope openkit|global|both] [--enable|--disabled] [--yes] [--json]`
...
- `openkit release sync-version` syncs generated release metadata from `package.json#version`; use it after manually editing the package version or prefer `openkit release prepare <version>` for the full release workflow.
...
- FEATURE-950 capability-platform maturation uses the existing status vocabulary and validation-surface labels through additive capability status envelopes. Phase completion records are stored as verification evidence details (`details.phase_completion`) and do not add lanes, workflow stages, approval gates, hidden MCP/skill activation, or live parallel execution.
