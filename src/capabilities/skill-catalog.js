...
function createEntry(name, options = {}) {
  const relativePath = options.path ?? `src/kit/skills/${name}/SKILL.md`;
...
function validateEntry(entry, { knownMcpIds }) {
  ...
  if (entry.packaging?.source === 'repo' && entry.path !== `src/kit/skills/${entry.name}/SKILL.md`) {
    errors.push(validationError(entry, 'path', 'Repo-backed skill path must match src/kit/skills/<name>/SKILL.md.'));
  }
